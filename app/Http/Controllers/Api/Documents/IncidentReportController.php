...
class IncidentReportController extends Controller
{
    ...
    private function detailPayload(DocumentIncidentReport $detail): array
    {
        return [
            ...
            'guard_user_ids'    => $detail->guards->pluck('id')->values(),
        ];
    }

    public function show(Request $request, Document $document)
    {
        abort_unless($document->document_type === 'feljegyzeses_jegyzokonyv', 404);
        $user = $request->user();
        ...
        return response()->json([
            ...
            'detail'   => $this->detailPayload($document->incidentReport),
        ]);
    }
    ...
    public function store(Request $request, DocumentSignatureService $signatureService, DocumentPdfService $pdfService)
    {
        ...
        $data = $request->validate([
            ...
            'guard_user_ids'            => ['required', 'array', 'min:1'],
            'guard_user_ids.*'          => ['integer', 'exists:tenant.users,id'],
        ...
        $document = DB::transaction(function () use ($data, $user, $tenant, $signatureService) {
            ...
            $detail->guards()->sync($data['guard_user_ids']);
