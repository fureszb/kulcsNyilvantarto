@extends('layouts.app')
@section('title', 'Napi Jelentés szerkesztése')

@section('content')
<div class="max-w-7xl mx-auto" x-data="securityEditForm()">
    ...
    <div class="relative overflow-hidden rounded-2xl mb-8 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 shadow-xl">
        <div class="absolute -top-16 -right-16 w-56 h-56 bg-amber-500/15 rounded-full blur-3xl pointer-events-none"></div>
        ...
        <div class="relative px-4 py-6 sm:px-8 sm:py-8 flex items-start justify-between gap-6 flex-wrap">
            <div>
                <p class="text-xs font-bold text-amber-400 uppercase tracking-widest mb-1">Biztonsági Szolgálat · Szerkesztés</p>
                <h1 class="text-2xl font-extrabold text-white tracking-tight sm:text-3xl">Napi Jelentés szerkesztése</h1>
                <div class="flex flex-wrap items-center gap-x-5 gap-y-1.5 mt-3 text-sm text-slate-400">
                    <span class="flex items-center gap-1.5">
                        ...
                        {{ $security->report_date->locale('hu')->translatedFormat('Y. F j., l') }}
                    </span>
                    <span class="flex items-center gap-1.5">
                        ...
                        Készítette: <strong class="text-slate-300 ml-0.5">{{ $security->prepared_by }}</strong>
                    </span>
                </div>
            </div>
            <div class="flex items-center gap-2.5 shrink-0">
                <a href="{{ route('security.show', $security) }}"
    ...
    <form method="POST" action="{{ route('security.update', $security) }}" @submit="prepareSubmit">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-5">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="report_date" value="{{ $security->report_date->toDateString() }}" required
                        ...
                        <div>
                            ...
                            <input type="text" name="prepared_by" value="{{ $security->prepared_by }}" placeholder="Teljes név" required
                                   class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-700 focus:border-rose-400 focus:bg-white focus:outline-none transition">
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white border border-rose-200 rounded-2xl shadow-sm">
                ...
                <div class="p-4 sm:p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        @include('security._person_picker', ['fieldName' => 'taken_over_from', 'currentValue' => $security->taken_over_from ?? '', 'placeholder' => 'Előző műszak vagyonőre…', 'required' => true])
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Átadás időpontja</label>
                        <input type="time" name="handover_time" value="{{ $security->handover_time }}"
                               class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-700 focus:border-rose-400 focus:bg-white focus:outline-none transition">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl shadow-sm">
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div class="relative">
                        @include('security._share_picker', ['sharedIds' => $sharedIds])
                    </div>
                    @endif
                    <div>
                        ...
                        <input type="text" name="cc_recipients"
                               value="{{ implode(', ', $security->cc_recipients ?? []) }}"
                        ...
                        <p class="text-xs text-slate-400 mt-1.5">Vesszővel elválasztva.</p>
                    </div>
                </div>
            </div>
...
function securityEditForm() {
    return {
        previous_shift_members: @json($security->previous_shift_members ?? []),
        service_members:        @json($security->service_members ?? []),
        equipment:              @json($security->equipment ?? []),
        inspectors:             @json($security->inspectors ?? []),
        patrols:                @json($security->patrols ?? []),
        incidents:              @json($security->incidents ?? []),
        events:                 @json($security->events ?? []),
        fire_alarms:            @json($security->fire_alarms ?? []),
        elevators:              @json($security->elevators ?? []),
        maintenance:            @json($security->maintenance ?? []),
        validationErrors: [],
        prepareSubmit(event) {
            const errors = [];
