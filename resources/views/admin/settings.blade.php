...
<div class="max-w-2xl space-y-6">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <form method="POST" action="{{ route('admin.settings.update') }}" class="px-6 py-6 space-y-5">
            ...
            {{-- Napi Jelentés email --}}
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1.5" for="security_notification_email">
                    Napi Jelentés – értesítési email(ek)
                </label>
                <input type="text" id="security_notification_email" name="security_notification_email"
                       value="{{ old('security_notification_email', $securityNotificationEmail) }}"
                       placeholder="biztonsag@pelda.hu, vezeto@pelda.hu"
                       class="w-full px-4 py-2.5 rounded-xl border @error('security_notification_email') border-red-400 bg-red-50 @else border-slate-200 bg-white @enderror text-slate-800 text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-rose-500/30 focus:border-rose-400 transition-colors">
                <p class="text-xs text-slate-400 mt-1.5">Vesszővel elválasztva több email-cím is megadható. Minden új napi jelentés automatikusan kiküldésre kerül.</p>
                @error('security_notification_email')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
