@extends('layouts.app')
@section('title', 'Vizsga – ' . $exam->title)

@php
    $stepsData = $exam->steps->map(fn($step) => [
        'id'            => $step->id,
...
@section('content')
<div x-data="examPlayer({{ $stepsData->toJson() }}, '{{ route('exam.result', $exam) }}', '{{ csrf_token() }}', {{ json_encode(auth('tenant')->user()->name) }}, {{ json_encode(auth('tenant')->user()->email ?? '') }})">
    ...
    <div class="relative overflow-hidden rounded-2xl mb-6 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 shadow-xl">
        ...
        <div class="relative px-8 py-7">
            <nav class="flex items-center gap-2 text-xs text-slate-500 mb-3">
                ...
                <a href="{{ route('exam.index') }}" class="hover:text-slate-300 transition-colors">Vizsgák</a>
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                <span class="text-slate-400">{{ $exam->title }}</span>
            </nav>
            <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    <h1 class="text-2xl font-extrabold text-white tracking-tight">{{ $exam->title }}</h1>
    ...
    <div x-show="!started" class="max-w-md mx-auto">
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-8">
            ...
            @if($exam->description)
                <p class="text-sm text-slate-500 mb-4 leading-relaxed">{{ $exam->description }}</p>
            @else
                    ...
                    <div class="max-w-2xl mx-auto bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-8">
                        ...
                        <template x-if="step.question_type === 'radio'">
                            <div class="space-y-3">
                                <template x-for="answer in step.answers" :key="answer.id">
                                    <button @click="selectRadio(answer.id, answer.is_correct)"
                                        :disabled="answered"
                                        :class="{
                                        ...
                                        <span class="w-5 h-5 rounded-full border-2 shrink-0"
                                              :class="answered && selectedAnswerId === answer.id ? 'border-amber-400 bg-amber-400' : 'border-slate-300'"></span>
                        ...
                        <template x-if="step.question_type === 'checkbox'">
                            <div class="space-y-4">
                                ...
                                <div class="space-y-3">
                                    <template x-for="answer in step.answers" :key="answer.id">
                                        <button @click="toggleCheckbox(answer.id)" :disabled="answered"
                                            :class="{
                                                'border-amber-400 bg-amber-50 text-amber-900': checkedAnswerIds.includes(answer.id),
                                            ...
                                            <span class="w-5 h-5 rounded-md border-2 shrink-0 flex items-center justify-center"
                                                  :class="checkedAnswerIds.includes(answer.id) ? 'border-amber-400 bg-amber-400' : 'border-slate-300'">
                                ...
                                <template x-if="!answered">
                                    <button @click="submitCheckbox()" :disabled="checkedAnswerIds.length === 0"
                        ...
                        <template x-if="step.question_type === 'text'">
                            <div class="space-y-3">
                                <input type="text" x-model="textInput"
                                       ...
                                       :disabled="answered"
                                       class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-800 font-mono placeholder-slate-400 focus:border-amber-400 focus:bg-white focus:outline-none transition"
                                       placeholder="Írja be a válaszát...">
                                <template x-if="!answered">
                                    <button @click="submitText()" :disabled="!textInput.trim()"
                                            class="w-full py-3 bg-amber-500 hover:bg-amber-600 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold rounded-xl transition-colors cursor-pointer">
                                        Válasz leadása
                                    </button>
                                </template>
            ...
            <div class="px-8 py-6"
                ...
                <div class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <h2 class="text-2xl font-extrabold text-white mt-0.5">{{ $exam->title }}</h2>
