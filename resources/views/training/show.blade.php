...
<div x-data="trainingPlayer({{ $stepsData->toJson() }}, '{{ route('training.result', $training) }}', '{{ csrf_token() }}')">
    ...
    {{-- Fejléc --}}
    <div class="w-full bg-white border border-slate-200 rounded-2xl px-8 py-5 mb-6 shadow-sm">
        <nav class="flex items-center gap-2 text-xs text-slate-400 mb-2">
            ...
            <a href="{{ route('training.index') }}" class="hover:text-slate-600 transition-colors">Oktatások</a>
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-slate-600 font-medium">{{ $training->title }}</span>
        </nav>
        <div class="flex items-center justify-between gap-4">
            <h1 class="text-xl font-extrabold text-slate-900">{{ $training->title }}</h1>
            <span class="text-sm text-slate-400 shrink-0" x-show="started && !completed"
                  x-text="`${currentStep + 1} / ${steps.length} lépés`"></span>
            <span class="text-sm font-semibold text-indigo-600 shrink-0" x-show="completed">Befejezve</span>
        </div>
        ...
        <template x-if="completed">
            <div class="mt-3 h-1.5 bg-green-100 rounded-full"><div class="h-full bg-green-500 rounded-full w-full transition-all duration-700"></div></div>
        </template>
    </div>
    ...
    <div x-show="!started" class="max-w-md mx-auto">
        <div class="card p-8">
            <div class="w-14 h-14 bg-indigo-100 rounded-2xl flex items-center justify-center mb-5 text-3xl">🎓</div>
            <h2 class="text-xl font-extrabold text-slate-900 mb-1">Oktatás megkezdése</h2>
            @if($training->description)
                <p class="text-sm text-slate-500 mb-6">{{ $training->description }}</p>
            @else
                <p class="text-sm text-slate-400 mb-6">{{ $stepsData->count() }} kérdéses oktatás. Az eredményt emailben is elküldjük.</p>
            @endif
            <div class="space-y-4">
                <div>
                    <label class="form-label">Teljes neve <span class="text-red-500">*</span></label>
                    <input type="text" x-model="participantName" @keydown.enter.prevent="startTraining()"
                           class="form-input" placeholder="Pl. Kovács János" autofocus>
                    <p x-show="nameError" class="text-red-500 text-xs mt-1">Adja meg a nevét a folytatáshoz.</p>
                </div>
                <div>
                    <label class="form-label">Email-cím <span class="text-xs text-slate-400">(opcionális – az eredményt ide is elküldjük)</span></label>
                    <input type="email" x-model="participantEmail" @keydown.enter.prevent="startTraining()" class="form-input" placeholder="nev@pelda.hu">
                </div>
                <div class="pt-1">
    ...
    <div x-show="completed" x-cloak class="space-y-5">
        <div class="card overflow-hidden">
            <div class="bg-gradient-to-r from-indigo-600 to-violet-600 px-8 py-6">
                <div class="flex items-start justify-between gap-4">
                    ...
                    <div class="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center shrink-0 text-3xl">🎓</div>
                </div>
        ...
        {{-- Email státusz --}}
        <div class="card px-6 py-4 space-y-2">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            ...
                            <th class="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Kérdés</th>
                            <th class="text-center px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider whitespace-nowrap">Kísérletek</th>
                            <th class="text-center px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Eredmény</th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr class="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-5 py-3.5 text-center">
                                    <span class="inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-bold"
                                          :class="r.attempts === 1 ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'"
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('training.index') }}" class="btn-secondary">← Vissza az oktatásokhoz</a>
            <button @click="restart()" class="btn-primary">Újra elvégzés</button>
        </div>
    </div>
                ...
                <div x-show="currentStep === idx" x-cloak>
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="card p-6 flex flex-col gap-5">
                            ...
                            {{-- Reveal (radio/checkbox: isCorrect; text: textCorrect) --}}
                        ...
                        {{-- Jobb: válasz UI (típustól függ) --}}
                        <div class="card p-6 flex flex-col justify-between gap-4">

                            {{-- RADIO --}}
                            <template x-if="step.question_type === 'radio'">
                                <div class="flex flex-col gap-4">
                                    <p class="text-sm font-semibold text-slate-500 uppercase tracking-wider">Válasszon egyet:</p>
                            ...
                            <template x-if="step.question_type === 'checkbox'">
                                <div class="flex flex-col gap-4">
                                    <p class="text-sm font-semibold text-slate-500 uppercase tracking-wider">Jelölje be az összes helyes választ:</p>
                                    <div class="flex flex-col gap-3">
                                        <template x-for="answer in step.answers" :key="answer.id">
                                            <button
                                    ...
                                    <template x-if="!isCorrect">
                                        <div class="flex flex-col gap-2">
                                            <button @click="checkCheckbox()"
                                                    ...
                                                    class="w-full py-3 bg-violet-600 hover:bg-violet-700 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold rounded-xl transition-colors">
                                                Ellenőrzés
                                            </button>
