...
export default function SecurityCreate({ sortedUsers, preparedBy, locations }: Props) {
    ...
    const [serviceMembers,       setServiceMembers]       = useState<ShiftRow[]>([
        { 'beosztás': 'Vagyonőr (diszpécser)', nev: '', 'idő_tól': '07:00', 'idő_ig': '07:00' },
        { 'beosztás': 'Vagyonőr (recepció)',   nev: '', 'idő_tól': '07:00', 'idő_ig': '19:00' },
    ]);
    const [previousShiftMembers, setPreviousShiftMembers] = useState<ShiftRow[]>([]);
    ...
    const [inspectors,  setInspectors]  = useState<InspRow[]>([{ neve: '', 'idő_tól': '', 'idő_ig': '' }]);
    const [patrols,     setPatrols]     = useState<PatrolRow[]>([{ 'vagyonőr': '', 'időpont': '', 'megjegyzés': '' }]);
    const [incidents,   setIncidents]   = useState<IncidentRow[]>([]);
    const [events,      setEvents]      = useState<EventRow[]>([]);
    const [fireAlarms,  setFireAlarms]  = useState<FireRow[]>([]);
    const [elevators,   setElevators]   = useState<ElevRow[]>([{ 'megnevezés': 'Liftek', 'időpont': '', 'ellenőrző': '' }]);
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        setValErrors([]);
        setProcessing(true);
        router.post(route('security.store'), {
    ...
    return (
        <AppLayout title="Új napi jelentés">
            ...
            <div className="relative overflow-hidden rounded-2xl mb-8 bg-gradient-to-br from-rose-950 via-slate-900 to-slate-900 shadow-2xl">
                ...
                <div className="absolute -bottom-16 -left-16 w-56 h-56 bg-rose-800/15 rounded-full blur-3xl pointer-events-none"/>
                ...
                <div className="relative px-4 py-6 sm:px-8 sm:py-8 flex items-start justify-between gap-6 flex-wrap">
                    <div>
                        <p className="text-rose-300/70 text-xs font-semibold uppercase tracking-widest mb-3 flex items-center gap-2">
                            <span className="inline-block w-1.5 h-1.5 rounded-full bg-rose-400"/>
                            Biztonsági napló › Új bejegyzés
                        </p>
                        <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight leading-tight">Új napi jelentés</h1>
                        <p className="text-slate-400 mt-2 text-sm">Töltse ki az összes releváns mezőt</p>
                    </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="bg-white border border-slate-200 rounded-2xl shadow-sm">
                    ...
                    <div className="p-6 space-y-5">
                        {sortedUsers.length > 0 && (
                            <SharePicker allUsers={sortedUsers} value={shareUserIds} onChange={setShareUserIds}/>
                        )}
                        <div>
                ...
                <div className="bg-white border border-slate-200 rounded-2xl shadow-sm px-4 py-4 sm:px-6 sm:py-5 flex items-center justify-between">
                    <a href={route('security.index')} className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 text-sm font-medium transition-colors">
                        Mégse
                    </a>
                    <button type="submit" disabled={processing}
                        className="inline-flex items-center gap-2 px-6 py-2.5 bg-rose-600 hover:bg-rose-500 disabled:bg-rose-400 text-white text-sm font-bold rounded-xl transition-colors shadow-sm cursor-pointer">
                        {processing ? (
