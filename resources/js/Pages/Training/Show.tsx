...
export default function TrainingShow({ training, stepsData, participantName = '', participantEmail = '' }: Props) {
    ...
    // video refs for autoplay-on-visible
    const mainVideoRef = useRef<HTMLVideoElement>(null);
    const revealVideoRef = useRef<HTMLVideoElement>(null);

    useEffect(() => {
        const videos = [mainVideoRef.current, revealVideoRef.current].filter(Boolean) as HTMLVideoElement[];
                        ...
                        {step.media_url && (
                            <div className="rounded-xl overflow-hidden bg-slate-100 flex items-center justify-center relative group">
                                {step.media_type !== 'video' ? (
                                    <img src={step.media_url} style={{ width: `${step.media_width ?? 100}%` }} className="max-h-[32rem] object-contain" alt="" />
                                ) : !isCorrect ? (
                                    ...
                                    <video ref={mainVideoRef} src={step.media_url} style={{ width: `${step.media_width ?? 100}%` }} className="max-h-[32rem] object-contain" controls controlsList="nodownload" loop />
                                )}
                                {step.media_type !== 'video' && (
                                    <button
                                        type="button"
                            ...
                            <div className="rounded-xl overflow-hidden border-2 border-green-300 bg-green-50">
                                <div className="px-3 py-2 bg-green-100 border-b border-green-200 flex items-center gap-1.5">
                                    ...
                                    {step.reveal_type !== 'video' && (
                                        <button
                                            type="button"
                                            onClick={() => setZoomItem({ url: step.reveal_url!, type: step.reveal_type ?? 'image' })}
                                            className="ml-auto w-7 h-7 rounded-lg bg-green-200 hover:bg-green-300 flex items-center justify-center text-green-700 transition-colors"
                                            title="Nagyítás"
                                        >
                                ...
                                {step.reveal_type === 'video' ? (
                                    <video ref={revealVideoRef} src={step.reveal_url} style={{ width: `${step.reveal_media_width ?? 100}%` }} className="max-h-96 object-contain" loop controls controlsList="nodownload" />
                                ) : (
                                    <img src={step.reveal_url} style={{ width: `${step.reveal_media_width ?? 100}%` }} className="max-h-96 object-contain" alt="" />
                                )}
                            </div>
                        )}
                    </div>
