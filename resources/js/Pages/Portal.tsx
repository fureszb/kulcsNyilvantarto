...
function SortableCard({
    def, badge, index,
}: { def: ModuleDef; badge?: number; index: number }) {
    ...
    return (
        ...
        >
            <div className={`group relative overflow-hidden bg-white border border-slate-200 rounded-2xl shadow-sm
                hover:shadow-xl ${ac.shadow} ${ac.border}
                motion-safe:hover:-translate-y-1 transition-all duration-300 flex flex-col
                ${isDragging ? 'shadow-2xl ring-2 ring-blue-200/50 !translate-y-0' : ''}`}>

                {/* Shimmer */}
                <div className={`absolute inset-0 -translate-x-full pointer-events-none z-10
                    bg-gradient-to-r from-transparent ${ac.shimmer} to-transparent
                ...
                <div
                    {...attributes}
                    {...listeners}
                    className="flex items-center justify-center py-2 cursor-grab active:cursor-grabbing select-none border-b border-slate-100 hover:bg-slate-50 transition-colors relative z-20"
                >
                ...
                >
                    {/* Icon */}
                    <div className={`relative w-14 h-14 rounded-2xl mb-6 flex items-center justify-center border
                        ${ac.iconBg} ${ac.iconHover} transition-all duration-300`}>
                    ...
                    <p className="text-slate-500 text-sm mt-2 flex-1 leading-relaxed">
                        {def.description}
                    </p>
                    ...
                    <div className="mt-6 pt-5 border-t border-slate-100 flex items-center justify-between">
                        <span className={`text-sm font-semibold ${ac.footerText} transition-colors`}>
                            {def.actionLabel}
                        </span>
                        <div className={`w-8 h-8 rounded-full border flex items-center justify-center
                            ${ac.arrowBg} ${ac.arrowHover} transition-all duration-300`}>
                            <svg className={`w-4 h-4 ${ac.arrowIcon} motion-safe:group-hover:translate-x-0.5 transition-all duration-200`}
                                 fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function Portal({ welcomeName, checksToday, trainingsCompleted, locations }: Props) {
    ...
    useEffect(() => {
        const activate = () => setAnimReady(true);
        if (welcomeName) {
            // Welcome overlay fully disappears at ~3300ms — wait for it
            const t = setTimeout(activate, 3350);
            return () => clearTimeout(t);
        }
        window.addEventListener('app-loader-done', activate, { once: true });
        if (!document.getElementById('page-loader')) activate();
        return () => window.removeEventListener('app-loader-done', activate);
    }, []);
    ...
    const visibleModules = ALL_MODULES.filter(m => !m.onlyNonPm || isNotPm);
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col">
            ...
            {showWelcome && welcomeName && (
                ...
                >
                    ...
                    <div className="relative text-center px-8">
                        ...
                        <h2 className="text-4xl sm:text-6xl font-extrabold tracking-tight" style={{ background: 'linear-gradient(135deg,#3b82f6,#60a5fa,#3b82f6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                            {welcomeName}!
                        </h2>
            ...
            {/* Scroll indicator */}
            <div className={`fixed bottom-6 left-1/2 z-40 flex flex-col items-center gap-2 pointer-events-none transition-all duration-700 ease-out
                ${!scrollHintReady
                    ? '-translate-x-1/2 translate-y-10 opacity-0'
                    : scrolled
                        ? '-translate-x-1/2 opacity-0'
                        : '-translate-x-1/2 opacity-100'}`}>
                <span className="text-xs font-semibold text-white bg-slate-900 px-3 py-1 rounded-full shadow-md whitespace-nowrap">
                    További modulok
                </span>
            ...
            <main className={`${animReady ? 'app-loaded' : ''} flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6`}>
                ...
                {/* Hero card */}
                <div className="app-page-enter relative overflow-hidden rounded-2xl mb-6 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 shadow-2xl" style={{ animationDelay: '0ms' }}>
                    ...
                    <div className="absolute inset-0 opacity-[0.025] pointer-events-none" style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,.3) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.3) 1px,transparent 1px)', backgroundSize: '32px 32px' }}/>
                    <div className="relative px-8 sm:px-10 py-10 sm:py-12 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                        <div>
                            {user && (
                            ...
                            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight leading-tight">{tenantName}</h1>
                        ...
                        <div className="flex flex-wrap gap-3">
                            ...
                            {nav?.newMessages != null && nav.newMessages + (nav?.newNotes ?? 0) > 0 && (
                                <div className="flex items-center gap-3 bg-white/10 border border-rose-400/20 rounded-xl px-4 py-3">
