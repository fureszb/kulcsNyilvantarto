...
export default function ExamShow({ exam, stepsData, participantName = '', participantEmail = '' }: Props) {
    const steps: StepData[] = stepsData ?? (exam.steps as StepData[] | undefined) ?? [];
    ...
    async function submitResult(finalAnswers: ExamAnswer[], finalScore: number) {
        ...
        try {
            ...
            await fetch(route('exam.result', exam.id), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken, Accept: 'application/json' },
                body: JSON.stringify({
                    results,
                    first_try_count: finalScore,
