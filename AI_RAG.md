...
> **Megjegyzés (modellválasztás, magyar nyelvű éles teszt alapján):** a `bge-m3` embedding többnyelvű, magyarra is erős. LLM-nek a **`gemma3:12b`** a nyertes (~8 GB VRAM): hibátlan magyar megfogalmazás, stabil táblázat-olvasás, elírás- és szinonima-tűrés, hallucináció-csapdákon 100%-os elutasítás. A `qwen2.5:14b` pontos, de magyartalan; a `llama3.1:8b` magyarja szép, de táblázatoknál megbízhatatlan.
...
3. **Kötelező szerveroldali szűrés minden vektorműveletnél.** A tudásbázis **cégszintű (tenantonként központi)**: a `retrieve_context` és a `delete_document` `must` filterrel (`tenant_id`) fut; szűrő nélküli lekérdezési útvonal a kódbázisban nem létezik. A payload mezőkön keyword indexek biztosítják, hogy a szűrés index-alapú legyen. **Szerepkörök:** a dokumentumokat kizárólag a tenant admin töltheti fel/törölheti/listázhatja; a dolgozók csak kérdezhetnek — nekik a rendszer sem fájllistát, sem forrásmegjelölést nem ad (a FastAPI dolgozói módban forrás-említés nélküli promptot használ és nem küld sources eseményt), az admin viszont minden üzenetnél látja a forrásokat.
